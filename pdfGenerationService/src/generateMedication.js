...
const medicationPublisher = require("./QueuePublisher/medicationPublisher");

const createMedication = (data) => {
  //create new document
  const doc = new pdfkit({ userPassword: "darsh123" });
  //save pdf file
  let fileloc = path.join(__dirname, "medication.pdf");
  ...
  function readAfter() {
    if (fs.existsSync(fileloc)) {
      ...
      const fileBuffer = fs.readFileSync(
        path.join(__dirname, "medication.pdf")
      );
  ...
  //call publisher
  medicationPublisher.publishMedication(pdf);
};
