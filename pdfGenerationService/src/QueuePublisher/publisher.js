...
const publishPDF = (pdf) => {
    ...
    async function connect(){
        try {
            ...
            const result = await channel.assertQueue("VisitDocument",{durable: true});
            console.log(typeof(pdf));
            console.log(pdf);
            channel.sendToQueue("VisitDocument",Buffer.from(JSON.stringify(pdf)),{persistent: true});
            logger.log({
                level: 'http',
